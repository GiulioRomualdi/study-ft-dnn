...
def get_dataset(device="cpu"):
    ...
    #         ft_name="r_leg_ft",
    #         device=device,
    #         max_accelerometer=training_data.max_accelerometer,
    #         max_gyro=training_data.max_gyro,
    ...
    training_data_accelerometer = training_data.accelerometer.cpu().numpy()
    training_data_gyro = training_data.gyro.cpu().numpy()
    training_data_expected = training_data.expected.cpu().numpy()

    # stack the data together
    X_training = np.hstack((training_data_raw_ft, training_data_accelerometer, training_data_gyro, training_data_temperature))
...
def train():
    ...
    # perform crossvalidation of the hyperparameters using `rbf` kernel
    param_grid = {
